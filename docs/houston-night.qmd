...
```{r}
# combine data, adding binary column for bo status
impacted <- impacted %>% mutate(blackout_status = "Impacted")
unimpacted <- unimpacted %>% mutate(blackout_status = "Unimpacted")

# combine data
full_bo_data <- rbind(impacted, unimpacted)


# 
# impacted_map <- ggplot() +
#   geom_sf(data = impacted, aes(fill = median_income)) +
#   scale_fill_gradient(low = '#D5F5E3', high = '#186A3B') +
#   labs(fill = "Median Income (Impacted)") +
#   theme_minimal()
# 
# impacted_map +
#   geom_sf(data = unimpacted, aes(fill = median_inc)) +
#   scale_fill_gradient(low = '#FDE9F4', high = '#8C0D50') +
#   labs(fill = "Median Income (Unimpacted)")
```
